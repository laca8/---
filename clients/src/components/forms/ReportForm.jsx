...
const ReportForm = () => {
  ...
  return (
    <div className="mt-3 font-bold">
      ...
      {loading && <Spinner />}
      {load && <Spinner />}
      <p className="font-bold text-right text-red-700 mb-1">
        ملاحظة (1) الاسئلة ذات النجمة الحمراء الرجاء ملؤها{" "}
        <span className="text-red-700">*</span>
      </p>
      <p className="font-bold text-right bg-cyan-700 p-1 w-auto rounded-sm">
        {" "}
        ملاحظة (2) الاسئلة ذات الخلفية الرمادية لا داعي لسؤالها{" "}
      </p>

      <div className="border-2 border-gray-400">
        <p className="bg-cyan-700 w-1/4 m-auto text-center p-1 mt-2">
          بيانات الحالة
        </p>
        ...
        <div className="grid grid-cols-4 max-sm:grid-cols-1 justify-evenly  text-right p-3 ">
          ...
          <div className="flex flex-col  w-auto p-1">
            ...
            <select
              ...
              {disData ? (
                disData?.map((x, i) => <option key={i}>{x?.title}</option>)
              ) : (
                <p>loading...</p>
              )}
            </select>
      ...
      <div className="border-2 border-gray-400 mt-2">
        ...
        <div className="grid grid-cols-4 max-sm:grid-cols-1 justify-evenly  text-right p-3  ">
          ...
          <div className="flex flex-col  w-auto p-1">
            ...
            <select
              ...
              {eduDataLevel ? (
                eduDataLevel?.map((x, i) => <option key={i}>{x?.title}</option>)
              ) : (
                <p>loading...</p>
              )}
            </select>
          ...
          <div className="flex flex-col  w-auto p-1">
            ...
            <select
              ...
              {typeDataWork ? (
                typeDataWork?.map((x, i) => <option key={i}>{x?.title}</option>)
              ) : (
                <p>loading...</p>
              )}
            </select>
          ...
          <div className="flex flex-col  w-auto p-1">
            ...
            <select
              ...
              {OutDataWork ? (
                OutDataWork?.map((x, i) => <option key={i}>{x?.title}</option>)
              ) : (
                <p>loading...</p>
              )}
            </select>
        ...
        <div className="flex flex-col p-3 text-right ">
          ...
          <textarea
            id="message"
            rows="4"
            className="bg-gray-800  rounded-sm w-full border-2 border-gray-400"
            value={order}
            onChange={(e) => setOrder(e.target.value)}></textarea>
        </div>
      </div>

      <div className="flex flex-row items-start gap-3 mt-2">
        <button
          className="bg-cyan-700 p-2 rounded-sm text-white w-32 mb-2"
