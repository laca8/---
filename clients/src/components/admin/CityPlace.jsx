      ...
      if (search1 != "" && search2 == "") {
        ...
        setData(
          res?.data?.data?.filter(
            (x) => x.city == search1 && x.place == search2
          )
        );
      } else {
        setData(res?.data?.data);
  ...
  return (
    <div className=" container flex flex-col  p-2 ">
      ...
      <div className="flex flex-row justify-between">
        <h1 className="p-2  rounded-sm text-white font-bold mb-2">المحافظة</h1>
        <button
          className="bg-cyan-700 p-1 text-white"
          onClick={() => handleD1()}>
          اضافة
        </button>
      </div>
      ...
      <div className="flex flex-row gap-2 p-1">
        <input
          className="p-1 border-2 bg-gray-800"
          type="text"
          value={search1}
                ...
                <div className="relative p-6 flex-auto">
                  <div className="flex flex-col mt-2 gap-4">
                    <input
                      ...
                      type="text"
                      placeholder="اضافة محافظة"
                      className="border-2 bg-gray-800 p-1 w-full"
                    />
                    <input
                      value={place}
                      onChange={(e) => setPlace(e.target.value)}
                      type="text"
                      placeholder="اضافة قسم/مركز"
                      className="border-2 bg-gray-800 p-1 w-full"
                    />
                      ...
                      {shiek?.map((x, i) => (
                        <div key={i} className="flex flex-row m-1">
                          <input
                            ...
                            type="text"
                            placeholder="شياخة"
                            className="border-2 bg-gray-800  w-full"
                          />

                          <button
                            className="bg-red-500 p-1 m-1 cursor-pointer"
                            onClick={() => handleRemove(i)}>
                            X
                          </button>
                          <button
                            disabled={!shiek}
                            className="bg-yellow-400  p-1 text-white m-1 h-10"
                            onClick={() => addInput()}>
                            +
                          </button>
                        </div>
      ...
      <div className=" font-bold  ">
        {data && (
          <div
            ...
            <table className="w-full text-sm text-center text-gray-500  mt-10 bg-blue-900 border-2 border-gray-400 shadow-lg ">
              ...
              <tbody className="bg-zinc-100 text-gray-700 ">
                {data?.map((x, i) => (
                  <tr key={i} className=" border-b-2 border-gray-950">
                    <th scope="row" className="p-2 font-medium  w-32">
                      <p className="bg-gray-800  text-center p-2 rounded-md text-white">
                        {x?.city}
                      </p>
                    </th>
                    ...
                    <td className=" flex flex-row flex-wrap gap-1 items-center content-center justify-center mt-1 ">
                      {x?.shiek.map((y, index) => (
                        <span
                          key={index}
                          className="bg-gray-800 text-center text-xs p-1  rounded-md text-white">
                          {y}
                        </span>
                      ))}
                    </td>
      ...
      {/* 
<div key={i} className='p-1 col-span-1 bg-gray-800 text-white text-sm mt-1 rounded-md shadow-md h-auto'>
                <div className='h-auto'>
                <input disabled className='p-1' value={`المحافظة : ${x?.city}`}  type='text'/>
                 
               { edit ?(
              <div className='bg bg-gray-800   mt-1'>
                ...
                
               {
                x?.shiek && x?.shiek?.map((y,i)=>(
                    ...
                    <span key={i}  className=' bg-gray-200 text-white p-1 rounded-md m-1'>{y}</span>
                ))
               }
                </div>
